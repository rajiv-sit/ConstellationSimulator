...
/**
 * SDP4 propagator for deep-space satellites (MEO/GEO).
 * Includes J2 + resonance secular effects.
 */
public class Sdp4Propagator extends AdvancedOrbitPropagator {

    public Sdp4Propagator(Tle tle) { super(tle); }
    ...
    protected double[] computePosition(double minutes) {
        ...
        double raanPerturbed = raan + raanRate*tSec + deepSpaceResonance(minutes, a);
        double wPerturbed = w + wRate*tSec + deepSpaceResonance(minutes, a);
    ...
    // Simple deep-space resonance correction
    private double deepSpaceResonance(double minutes, double a) {
        if(a < 20000) return 0;
        return 1e-6 * Math.sin(minutes / 1440.0 * 2*Math.PI);
    }
